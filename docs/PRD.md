...
MVP 的 GitHub PR 导入和规则分析在前端完成；AI 代码评审通过后端接口完成。页面提供第三方大模型配置入口，支持协议选择、OpenAI-compatible `BASE_URL`、`API_KEY`、`MODEL`。GitHub PR 导入逻辑放在 `src/lib/githubPullRequest.ts`，核心规则 Review 逻辑放在 `src/lib/reviewEngine.ts`，AI 评审逻辑放在 `server/aiReviewCore.mjs`。
